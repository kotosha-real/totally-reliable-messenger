 ...
 * @returns {array}
 */
export const range = function (...args: any[]): any[] {
  const index: number = args.findIndex((arg) => typeof arg === 'boolean')
  const isRight: any = index !== -1 ? args.splice(index, 1) : false

  args = args.filter((arg) => arg !== undefined)

  const start: number = args.length > 1 ? args[0] : 0
  const end: number = args.length > 1 ? args[1] : args[0] ? args[0] : 0
  let step: number = args.length === 3 ? args[2] : 1
